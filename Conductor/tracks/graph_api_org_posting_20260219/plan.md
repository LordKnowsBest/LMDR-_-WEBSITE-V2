# Graph API Organic Posting — Implementation Plan
...
## Phase 0 — Research & Decisions ✅ (Complete — pre-work in spec.md)
...
- [x] **0.1** Document FB Page feed posting endpoint, parameters, and required permissions
  ...
  - Source: [Graph API — Page Feed Publishing](https://developers.facebook.com/docs/graph-api/reference/page/feed/)
- [x] **0.2** Document IG content publishing two-step flow (container → publish)
  - Step 1: `POST /{ig-id}/media` — create container
  - Step 2: `POST /{ig-id}/media_publish` — publish container
...
- [x] **0.4** Document long-lived token acquisition flow
  - Short-lived User → Long-lived User (~60 days) → Long-lived Page (no expiry*)
...
- [x] **0.5** Document IG account prerequisites (Business/Creator + linked FB Page)
  - Source: [Instagram API Overview — Requirements](https://developers.facebook.com/docs/instagram-api/guides/content-publishing#requirements)
- [x] **0.6** Confirm what is NOT possible (profile posting, Stories via Graph API, Shopping tags)
  - Documented in spec.md Section 9 — Out of Scope
...
**Acceptance Criteria:**
- spec.md contains doc-backed capability matrix, preconditions, auth model, posting flows, and error taxonomy ✅
- Architecture decision documented with rationale ✅

**Kill Switch:** N/A — this phase is documentation only.

---

## Phase 1 — MVP: Minimal Working Prototype

> Goal: One successful organic FB Page text post and one IG image post, end-to-end, triggered manually from backend.

### 1.1 — Secrets & Token Bootstrap

- [ ] **1.1.1** Create `src/backend/socialSecretService.jsw`
...
- [ ] **1.1.2** Manually run the token acquisition flow to populate Wix Secrets Manager
  - Step a: FB Login for Business → get short-lived User token
  - Step b: Server-side exchange → long-lived User token (`GET /oauth/access_token?grant_type=fb_exchange_token`)
  - Step c: Get Page + IG tokens → `GET /{user-id}/accounts`
  - Step d: Add each token to Wix Secrets Manager with correct key names
- [ ] **1.1.3** Write `src/backend/socialTokenService.jsw`
  - Export: `validateToken(token)` — calls `GET /debug_token` and returns `{ is_valid, expires_at, scopes }`
  - Export: `exchangeForLongLived(shortLivedToken)` — exchanges using app credentials (server-side only)
  - Export: `getPageTokens(longLivedUserToken)` — calls `GET /{user-id}/accounts` and returns page+IG token pairs
...
### 1.2 — Facebook Page Text Post

- [ ] **1.2.1** Create `src/backend/socialPostingService.jsw` with FB posting function:
  ```javascript
...
- [ ] **1.2.2** Add `SOCIAL_POSTING_ENABLED: false` to `src/backend/configData.js` (starts disabled; flip to `true` for testing)
- [ ] **1.2.3** Write test: `src/public/__tests__/socialPostingService.test.js`
  - Test: FB text post success (mocked API response `{ id: "page_post_123" }`)
  - Test: FB post fails with token expired (OAuthException code 190) → returns `TOKEN_EXPIRED` error type
...
### 1.3 — Instagram Image Post (Container + Publish)

- [ ] **1.3.1** Add `postToInstagram(payload)` to `socialPostingService.jsw`:
  ```javascript
  export async function postToInstagram(payload) {
    // payload: { igUserId, image_url, caption?, dedupe_key }
  }
  ```
  - Pre-check: `GET /{ig-id}/content_publishing_limit` — reject if `quota_usage >= 50`
...
- [ ] **1.3.2** Add IG tests to `socialPostingService.test.js`:
  - Test: IG image post success (container created → published)
  - Test: Pre-check blocks when `quota_usage=50`
  - Test: IG post fails at container creation → `MEDIA_NOT_PUBLIC` error type
  - Test: IG post fails at publish step → `MEDIA_PROCESSING_FAILED` error type
...
- [ ] **1.4.1** Log each post attempt to `observabilityService.jsw` with fields: `platform`, `status`, `post_id / media_id`, `error_type` (if any), `duration_ms`
- [ ] **1.4.2** Console-log all token validation failures with masked token (first 6 + last 4 chars only)
...
- Set `SOCIAL_POSTING_ENABLED: false` in `configData.js` — immediately prevents all posting
- No rollback of already-published posts needed (they must be deleted manually via Meta UI if needed)

---

## Phase 2 — Posting Service Architecture

> Goal: Unified post dispatcher, full post type support, post queue data model.

### 2.1 — Unified Post Dispatcher

- [ ] **2.1.1** Refactor `socialPostingService.jsw` into a clean dispatcher:
  ```javascript
  export const post = {
  ...
  ```
- [ ] **2.1.2** Implement `postToBoth(payload)`:
  - Dedupe check before dispatch
  - `Promise.allSettled([post.facebook(), post.instagram()])`
  - Evaluate: both success → `published`; one fail → `partial`; both fail → `failed`
  - Return: `{ facebook: result, instagram: result, overall_status }`

### 2.2 — Full FB Post Type Support

- [ ] **2.2.1** Add `post_type: 'link'` to `postToFacebook()`:
  - Body: `{ message?, link }` — note: at least one must be provided
  - Validate `link` is a valid URL
- [ ] **2.2.2** Add `post_type: 'photo'` to `postToFacebook()`:
  - Use `POST /v25.0/{page-id}/photos` with `{ url, caption? }`
- [ ] **2.2.3** Add `post_type: 'video'` to `postToFacebook()`:
  - Use `POST /v25.0/{page-id}/videos` with `{ file_url, description? }`
- [ ] **2.2.4** Add `post_type: 'scheduled'` flag support:
  - Body includes `published: false, scheduled_publish_time: {unix_ts}`
  - Validate `scheduled_publish_time` is 10 min – 75 days in future

### 2.3 — Full IG Post Type Support

- [ ] **2.3.1** Add `post_type: 'video'` to `postToInstagram()`:
  - Body: `{ video_url, media_type: 'VIDEO', caption? }`
  - Poll `status_code` until `FINISHED` or `ERROR` (max 10 polls, 5s interval)
- [ ] **2.3.2** Add `post_type: 'reels'` to `postToInstagram()`:
...
- [ ] **2.3.3** Add `post_type: 'carousel'` to `postToInstagram()`:
...
### 2.4 — Post Queue Data Model

- [ ] **2.4.1** Create Airtable table `v2_Social Post Queue` with fields per spec.md Section 10
- [ ] **2.4.2** Create `src/backend/socialQueueService.jsw`:
  - `createQueueRecord(payload)` — creates record with `status=queued`
  - `updateQueueRecord(id, updates)` — updates status, post_id, error fields
  - `getByDedupeKey(dedupe_key)` — fetches existing record for idempotency
  - `getFailedRecords()` — returns records with `status=failed` and `retry_count < 3`
- [ ] **2.4.3** Integrate queue into `post.facebook()` and `post.instagram()`: create record before dispatch, update after

### 2.5 — Idempotency

- [ ] **2.5.1** Implement `generateDedupeKey(payload)` utility:
  - SHA-256 of `platform + post_type + (caption || message) + (image_url || link || '') + account_id`
- [ ] **2.5.2** Check `getByDedupeKey()` at start of every post method — abort if `status=published`
...
## Phase 3 — Scheduling & Rate Limit Enforcement

> Goal: Scheduled posting that does not rely on fragile cron jobs; proactive rate limit enforcement.

### 3.1 — Scheduling Mechanism

- [ ] **3.1.1** Decision: Use **Facebook's native scheduling** (`published=false + scheduled_publish_time`) for FB posts — avoiding any Wix cron dependency for FB scheduling.
- [ ] **3.1.2** For Instagram (no native scheduling via API): implement an in-queue scheduling mechanism:
...
- [ ] **3.1.3** Create `src/backend/socialQueueJob.jsw`:
  - `processSocialQueue()` — main job handler
  - Exported for use in `jobs.config`

### 3.2 — Rate Limit Guard

- [ ] **3.2.1** Create `src/backend/socialRateLimitService.jsw`:
  - `checkIGQuota(igUserId)` — calls `content_publishing_limit` and returns `{ can_post, quota_used, quota_total, quota_duration }`
  - `checkFBUsage(pageId)` — reads `X-App-Usage` header from last response; returns alert if any metric > 75%
  - Cache last quota check in-memory for 60s to avoid excessive API calls
- [ ] **3.2.2** All IG post methods call `checkIGQuota()` before container creation; reject if `can_post=false` with `RateLimitError`
- [ ] **3.2.3** Log rate limit status to observability on every queue run
...
> Goal: Production-grade reliability — retries, dead-letter, audit logs, token health monitoring.

### 4.1 — Structured Retry with Dead-Letter

- [ ] **4.1.1** Implement retry logic in `socialQueueJob.jsw`:
  - For retryable errors (NETWORK_TIMEOUT, CONTAINER_EXPIRED, RATE_LIMITED_FB): increment `retry_count`, re-queue with backoff delay
  - Backoff: attempt 1 → +5min, attempt 2 → +15min, attempt 3 → +45min
  - After `retry_count >= 3`: move to `status=dead_letter`; alert admin via `emailService.sendAlert()`
- [ ] **4.1.2** For non-retryable errors (TOKEN_EXPIRED, PERMISSION_MISSING, MEDIA_PROCESSING_FAILED): immediately set `status=failed`, alert admin, do NOT increment retry_count
- [ ] **4.1.3** Create `getDead LetterQueue()` in `socialQueueService.jsw` for admin review

### 4.2 — Token Health Monitoring

- [ ] **4.2.1** Create `src/backend/socialTokenHealthJob.jsw`:
...
- [ ] **4.2.2** Add nightly job to `jobs.config`: `0 2 * * *` (2 AM UTC)
...
### 4.4 — Admin Alerting Hooks

- [ ] **4.4.1** Add `sendSocialAlert(alertType, details)` to `emailService.jsw`
...
- [ ] **4.4.2** Wire all error paths in `socialPostingService.jsw` to call `sendSocialAlert()`
...
## Phase 5 — Migration-Ready Improvements

> Goal: Ensure the posting infrastructure is portable to GCP when the platform migrates.

### 5.1 — Secrets Provider Abstraction

- [ ] **5.1.1** Refactor `socialSecretService.jsw` to use a provider interface:
  ```javascript
  const SecretProvider = process.env.RUNTIME === 'gcp'
    ? GCPSecretProvider
    : WixSecretProvider;
  ```
- [ ] **5.1.2** Implement `WixSecretProvider` (wraps current `getSecret()` calls — no behavior change)
- [ ] **5.1.3** Stub `GCPSecretProvider` with a TODO pointing to `gcp_migration_20260218` track for implementation
- [ ] **5.1.4** Document the migration steps in `spec.md` Section 5 (already written) — verify they remain accurate

### 5.2 — Cloud Run Portability Notes

- [ ] **5.2.1** Ensure all post functions are pure HTTP calls with no Wix-specific dependencies (no `wix-fetch` — use standard `fetch`; no Wix session context)
...
- [ ] **5.2.3** Add a `SOCIAL_RUNTIME` config flag to `configData.js` for future runtime targeting

### 5.3 — Multi-Tenant Readiness (Future)

- [ ] **5.3.1** Verify all functions accept `pageId` and `igUserId` as explicit parameters (no hardcoded IDs)
- [ ] **5.3.2** Verify secret keys use `_{account_id}` suffix pattern — ready for multiple clients
- [ ] **5.3.3** Document multi-tenant considerations in spec.md Section 5 (token scope separation confirmed ✅)
...
- All post functions accept `pageId` / `igUserId` explicitly — no hardcoded account IDs in service logic
...
- [ ] Full test suite passing: `socialPostingService.test.js`, `socialQueueService.test.js`, `socialRateLimitService.test.js`, `socialTokenService.test.js`
- [ ] Zero hardcoded page/IG IDs in service files
