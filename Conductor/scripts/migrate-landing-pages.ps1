...
$files = Get-ChildItem "src/public/landing/*.html"
...
foreach ($file in $files) {
    ...
    # Using specific lookahead to ensure we target the right script
    ...
    if ($content -notmatch 'lmdr-config\.js') {
        ...
        $content = $content -replace '(<script src="https://cdn.tailw.*?></script>)', $replacement
    }
