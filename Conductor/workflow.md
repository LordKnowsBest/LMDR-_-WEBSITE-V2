...
7. **Evidence Pack Verification** *(for tracks with frontend pages only)*:
   - If the task includes HTML pages in `src/public/`, run the DevTools Evidence Pack verification.
   - Execute: `.claude/agents/evidence-pack.md`
   - Confirm: `quality_gate.json` shows `pass: true`
   - Attach: `run_id` to track's `metadata.json` in `verification_run` field
