...
if ($input.tool_input.content) {
    # write_file tool - check the new content
    $content = $input.tool_input.content
} elseif ($input.tool_input.new_string) {
    # replace tool - we'd need to check the resulting file
    # For replace tool, we'll check the file after edit in PostToolUse
    exit 0
}
