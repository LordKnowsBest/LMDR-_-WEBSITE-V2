  ...
  function buildCommandBar() {
    ...
    <div id="aos-cmd-bar" class="fixed bottom-20 left-1/2 -translate-x-1/2 w-[90%] max-w-2xl px-4 py-3 rounded-2xl border border-white/20 dark:border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.12)] z-30 flex flex-col gap-2 transition-all">
      <div class="flex items-center justify-center gap-1.5 overflow-x-auto no-scrollbar pb-1">
        <button onclick="if(AOS.chat)AOS.chat.sendCommandBar('Run Diagnostics')" class="whitespace-nowrap text-[10px] font-bold text-lmdr-dark/60 dark:text-solar-bg/70 bg-white/40 dark:bg-white/10 px-3 py-1.5 rounded-full border border-black/5 dark:border-white/5 hover:bg-white dark:hover:bg-white/20 transition-all shadow-sm">Run Diagnostics</button>
        <button onclick="if(AOS.chat)AOS.chat.sendCommandBar('Audit AI Spend')" class="whitespace-nowrap text-[10px] font-bold text-lmdr-dark/60 dark:text-solar-bg/70 bg-white/40 dark:bg-white/10 px-3 py-1.5 rounded-full border border-black/5 dark:border-white/5 hover:bg-white dark:hover:bg-white/20 transition-all shadow-sm">Audit AI Spend</button>
        <button onclick="if(AOS.chat)AOS.chat.sendCommandBar('Find Carrier')" class="whitespace-nowrap text-[10px] font-bold text-lmdr-dark/60 dark:text-solar-bg/70 bg-white/40 dark:bg-white/10 px-3 py-1.5 rounded-full border border-black/5 dark:border-white/5 hover:bg-white dark:hover:bg-white/20 transition-all shadow-sm">Find Carrier</button>
        <button onclick="if(AOS.chat)AOS.chat.sendCommandBar('View Logs')" class="whitespace-nowrap text-[10px] font-bold text-lmdr-dark/60 dark:text-solar-bg/70 bg-white/40 dark:bg-white/10 px-3 py-1.5 rounded-full border border-black/5 dark:border-white/5 hover:bg-white dark:hover:bg-white/20 transition-all shadow-sm">View Logs</button>
        <button onclick="if(AOS.chat)AOS.chat.sendCommandBar('Feature Flags')" class="whitespace-nowrap text-[10px] font-bold text-lmdr-dark/60 dark:text-solar-bg/70 bg-white/40 dark:bg-white/10 px-3 py-1.5 rounded-full border border-black/5 dark:border-white/5 hover:bg-white dark:hover:bg-white/20 transition-all shadow-sm">Feature Flags</button>
      </div>
      <div class="flex items-center gap-2.5 bg-white/60 dark:bg-black/40 p-1.5 rounded-xl shadow-inner border border-black/5 dark:border-white/5">
        ...
        <input type="text" id="aos-cmd-input" class="flex-1 bg-transparent border-none outline-none text-[13px] text-lmdr-dark dark:text-solar-bg placeholder-lmdr-dark/40 dark:placeholder-solar-bg/30 font-medium px-1" placeholder="Ask AdminOS anything — try 'show system health'..." onkeydown="if(event.key==='Enter' && AOS.chat) AOS.chat.sendCommandBar(this.value)">
        <button class="w-8 h-8 rounded-lg bg-solar-blue flex items-center justify-center text-white shrink-0 shadow-md transition-transform active:scale-95" onclick="const i=document.getElementById('aos-cmd-input'); if(AOS.chat && i.value) AOS.chat.sendCommandBar(i.value)">
          <span class="material-symbols-outlined text-[16px]">arrow_upward</span>
        </button>
      </div>
