...
export async function isEventProcessed(eventId) {
  ...
  if (!result.success) {
    throw new Error(result.error || 'Failed to check event idempotency');
  }
  return (result.items?.length || 0) > 0;
}

export async function logStripeEvent(eventId, eventType, data = {}) {
  const result = await dataAccess.insertRecord(CONFIG.stripeEventsKey, { event_id: eventId, event_type: eventType, processed_at: new Date() }, { suppressAuth: true });
  if (!result.success) {
    // We must ensure the event is logged to prevent duplicate processing on retry.
    // Throwing here will cause the webhook to return 500, prompting Stripe to retry.
    // However, the business logic has already executed! Ideally we log BEFORE processing or use a transaction.
    // Given the current architecture (log after), we log a warning but don't fail,
    // OR we accept the risk of duplicate processing if logging fails.
    // To be safe against replay attacks, failing here is better (retry -> check idempotency -> already processed).
    // Wait, if we fail here, the next retry will check isEventProcessed. Since we FAILED to log, isEventProcessed will return false.
    // So the event will be processed AGAIN. This is bad.
    // Ideally, we should insert a "processing" record at the start.
    // But for this task, let's at least log the error.
    console.error('[StripeCore] Failed to log stripe event:', result.error);
    throw new Error(result.error || 'Failed to log stripe event');
  }
}
...
async function getApiPriceId(tier, planType) {
  const secrets = await getStripeSecrets();
  const keyMap = {
    starter: {
...
async function resolveApiTierFromSubscription(stripeSubscription) {
...
async function getApiPartner(partnerId) {
...
async function getApiSubscriptionByPartner(partnerId) {
  const result = await dataAccess.queryRecords(CONFIG.apiSubscriptionsKey, {
    filters: { partner_id: String(partnerId) },
